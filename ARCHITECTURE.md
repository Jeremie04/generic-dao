...
- **`CollectionRelationLoader`** — charge les champs `List<Item>`/`Item[]` (relation "un-à-plusieurs", voir README) activés via `setFetchRelations`. Volontairement **pas** un `JOIN` : vu que `ResultSetMapper` mappe une ligne sur un objet, un `JOIN` classique dupliquerait chaque parent une fois par enfant. À la place : une requête batch par relation demandée (`SELECT * FROM enfant WHERE fk IN (id1, id2, ...)`, groupée pour tous les parents déjà chargés, pas un aller-retour par parent), puis un regroupement en mémoire par clé étrangère. Coût réel : une requête supplémentaire par relation `setFetchRelations`, sur une colonne FK qui doit être indexée pour rester bon marché (un scan d'index, comparable au coût de la requête principale) ; contrepartie identifiée : la longueur de la clause `IN` variant avec le nombre de parents, le cache de *prepared statements* d'HikariCP (`cachePrepStmts`) ne peut pas réutiliser un plan d'un appel à l'autre pour cette requête précise (pas un problème pour un nombre de parents modéré, à surveiller si un `select()` peut ramener des dizaines de milliers de lignes).
